...
public class WireHandler {
	...
	public WireHandler(World world) {
		this.world = world;
	...
	private Node getNextNode(BlockPos pos, BlockState state) {
		return state.is(Blocks.WIRE_REDSTONE) ? new WireNode(world, pos, state) : getNextNode().set(pos, state, true);
	}
	...
	private Node revalidateNode(Node node) {
		...
		boolean isWire = state.is(Blocks.WIRE_REDSTONE);
...
//			profiler.end();
	...
	private void updateBlock(Node node, Block<?> neighborBlock) {
		...
		if (!state.isAir() && !state.is(Blocks.WIRE_REDSTONE)) {
